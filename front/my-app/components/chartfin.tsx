...
const ChartComponent = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/consumption/Industrie_FINALE.csv/${year}`)
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/consumption/Residentiel_FINALE.csv/${year}`)
      .then(response => response.json())
      .then(data => {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/consumption/Transport_FINALE.csv/${year}`)
      .then(response => response.json())
      .then(data => {
...
function BarChart({ gasData, electricityData, liquidData }: { gasData: number, electricityData: number, liquidData: number }) {
  return (
    <div style={{ height: '400px' }}>
      <ResponsiveBar
        data={[
          { name: "Industrie", count: gasData },
          { name: "Residentiel", count: electricityData },
          { name: "Transport", count: liquidData },
        ]}
        keys={["count"]}
        indexBy="name"
